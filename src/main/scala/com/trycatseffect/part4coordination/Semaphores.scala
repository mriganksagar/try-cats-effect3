...
object Semaphores extends IOApp.Simple {
    ...
    def loginAndCompute[T](id: Int, sem: Semaphore[IO], aTask: IO[T]): IO[T] = for {
        _ <- IO(s"[SessionId: $id], starting ").ownDebug
        _ <- sem.acquire
        // critical section
        _ <- IO(s"[SessionId: $id], logged-in and computing").ownDebug
        result <- aTask
        _ <- IO(s"[SessionId: $id], computed stuff and logging out").ownDebug
        _ <- sem.release
    } yield result

    def loginAndComputeWeighted[T](id: Int, wt: Int, sem: Semaphore[IO], aTask: IO[T]): IO[T] =
        for {
            _ <- IO(s"[SessionId: $id], starting to acquire $wt resources").ownDebug
            _ <- sem.acquireN(wt)
            // critical section
            _ <- IO(s"[SessionId: $id], logged-in and computing").ownDebug
            result <- aTask
    ...
    override def run = program_20usersLoggingIn.ownDebug.void
}
